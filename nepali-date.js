...
function nepaliToEnglish(nepYear, nepMonth, nepDay) {
    ...
    // Add days from reference year to nepYear - 1
    ...
    // Add days from months in nepYear before nepMonth
    for (let month = 0; month < nepMonth - 1; month++) {
        totalDays += nepaliDateData[nepYear][month];
    }
    
    // Add days in nepMonth
    totalDays += nepDay - 1; // Subtract 1 because refDateNepali.day is 1 (not 0)
    
    // Calculate English date by adding total days to reference English date
...
function englishToNepali(engDate) {
    ...
    let remainingDays = diffDays + 1; // Add 1 because refDateNepali.day is 1 (not 0)
    ...
    // Remaining days is the day in the month
...
function formatNepaliDate(nepYear, nepMonth, nepDay) {
    return `${nepYear} ${nepaliMonths[nepMonth - 1]} ${nepDay}`;
}
